        ...
        for (int b_i = 0; b_i < Bs.size(); b_i++) {
          for (int c_i = 0; c_i < Cs.size(); c_i++) {
            ...
            for (int a_i = 0; a_i < As.size(); a_i++) {
              ...
              cout << "check " <<  As[a_i].first << endl;
              int delIndex = -1;
              bool add = true;
              for (int a = 0; a <  CYKTable[begin - 1][end - 1].size() ; a++) {
                if (As[a_i].first == CYKTable[begin - 1][end - 1][a].nonTerm && prob <= CYKTable[begin - 1][end - 1][a].prob) {
                  add = false;
                  break;
                }
                else if (As[a_i].first == CYKTable[begin - 1][end - 1][a].nonTerm) {
                    delIndex = a;
                }
              }
              if (add) {
                  if (delIndex > 0) {
                    cout << "erase " << CYKTable[begin - 1][end - 1][delIndex].nonTerm << endl;
                    CYKTable[begin - 1][end - 1].erase(CYKTable[begin - 1][end - 1].begin() + delIndex);
                  }
                  sAdded = true;
          ...
          while (k < CYKTable[begin - 1][end - 1].size()) { // for each entry in table (note CYKTable may grow inside loop)
            while (added) {
              ...
              for (int l = 0; l < LHSsRec.size(); l++) { // for all B's
                ...
                cout << "check " << lhsCurrentEntry.first << endl;
                bool add = true;
                int delIndex  = -1;
                int a  = 0;
                for (a ; a <  CYKTable[begin - 1][end - 1].size() ; a++) {
                  cout << "nonTerm: " << CYKTable[begin - 1][end - 1][a].nonTerm << endl;
                  if (CYKTable[begin - 1][end - 1][a].nonTerm == lhsCurrentEntry.first && recProb <= CYKTable[begin - 1][end - 1][a].prob) {
                    add = false;
                    break;
                  }
                  else if (CYKTable[begin - 1][end - 1][a].nonTerm == lhsCurrentEntry.first){
                    delIndex = a;
                  }
                }
